...
public class CompetitionHeatAthlete extends BaseEntity {
 ...
 @ManyToOne(fetch=FetchType.LAZY,optional=false) @JoinColumn(name="athlete_id",nullable=false)
 private CompetitionAthlete athlete;
 @Column(name="lane_number",nullable=false) private Integer positionNumber;
 @Enumerated(EnumType.STRING) @Column(name="check_in_status",nullable=false,length=30)
 private CheckInStatus checkInStatus;
 @Column(name="check_in_time") private LocalDateTime checkInTime;
 @OneToOne(mappedBy="heatAssignment",cascade=CascadeType.ALL,orphanRemoval=true)
 private CompetitionJudgeAssignment judgeAssignment;
}
